...
from ..providers.model_providers.project_provider import ProjectProvider
...
class ProjectReader(RawSqlQueryReader):
    ...
    _lookup_table_name = "core_application_project"
    """
    ...
    def initialize_query_builder(self):
        ...
        """
        self.items_builder\
            .add_select_expression("core_application_project.id")\
            .add_select_expression("core_application_project.alias")\
            .add_select_expression("core_application_project.avatar")\
            .add_select_expression("core_application_project.name")\
            .add_select_expression("core_application_project.description")\
            .add_select_expression("core_application_project.project_dir")\
            .add_select_expression("core_application_project.unix_group")\
            ...
            .add_data_source("core_application_project")\
            .add_order_term("core_application_project.name")
        self.items_builder.data_source\
            .add_join(
                self.items_builder.JoinType.INNER,
                SqlTable("core_application_group", "root_group"),
                "ON (root_group.id=root_group_id)")\
            .add_join(
                self.items_builder.JoinType.INNER,
                SqlTable("core_application_groupuser", "governor_info"),
                "ON (governor_info.group_id=root_group_id AND governor_info.is_governor)")\
            .add_join(
                self.items_builder.JoinType.INNER,
                SqlTable("core_application_user", "governor"),
    ...
    def apply_name_filter(self, project_name):
        ...
        for builder in [self.count_builder, self.items_builder]:
            builder.main_filter &= SearchQueryFilter("core_application_project.name", project_name,
                                                     is_prepared=True, must_start=True)

    def apply_root_group_filter(self, root_group):
        """
        Changes the SQL query in such a way as it retrieves project which name starts from a given string
        ...
        for builder in [self.count_builder, self.items_builder]:
            builder.main_filter &= \
                StringQueryFilter("core_application_project.root_group_id=%s", root_group.id)
    ...
    def apply_user_filter(self, user):
        ...
        for builder in [self.items_builder, self.count_builder]:
            builder.data_source\
                .add_join(
                    ...
                    SqlTable("core_application_groupuser", "root_user"),
                ...
                .add_join(
                    builder.JoinType.LEFT,
                    SqlTable("core_application_group", "acl_group"),
                    "ON (acl_group.id=acl.group_id)")\
                .add_join(
                    builder.JoinType.LEFT,
                    SqlTable("core_application_groupuser", "acl_user"),
        ...
        self.count_builder.select_expression = \
            self.count_builder.select_total_count("core_application_project.id", distinct=True)
        ...
        self.items_builder\
            ...
            .add_group_term("core_application_project.id")\
