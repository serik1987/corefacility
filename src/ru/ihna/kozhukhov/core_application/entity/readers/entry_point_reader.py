...
from ..providers.model_providers.entry_point_provider import EntryPointProvider
...
class EntryPointReader(RawSqlQueryReader):
    ...
    _lookup_table_name = "core_application_entrypoint"
    """
    ...
    def initialize_query_builder(self):
        ...
        """
        self.items_builder\
            .add_select_expression("core_application_entrypoint.id")\
            .add_select_expression("core_application_entrypoint.entry_point_class")\
            .add_select_expression("core_application_entrypoint.alias")\
            .add_select_expression("core_application_entrypoint.type")\
            .add_select_expression("core_application_entrypoint.name")\
            .add_select_expression("belonging_module.app_class")\
            .add_order_term("core_application_entrypoint.name")

        self.count_builder\
            .add_select_expression(self.count_builder.select_total_count())

        for builder in (self.items_builder, self.count_builder):
            builder.add_data_source("core_application_entrypoint")
            builder.data_source.add_join(builder.JoinType.LEFT,
                                         SqlTable("core_application_module", "belonging_module"),
