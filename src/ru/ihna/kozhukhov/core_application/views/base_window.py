...
class BaseWindow(SetCookieMixin, TemplateView):
    ...
    @property
    def _short_app_module(self):
        """
        A short application module
        """
        short_module = self._app_module.rsplit('.')[-1]
        if short_module.endswith('_application'):
            short_module = short_module.split('_application')[0]
        return short_module
