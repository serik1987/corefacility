...
/** Allows user to adjust settings for the Cookie authorization
 * 
 ...
 * 									will be provided.
 */
export default class CookieAuthorizationModuleForm extends ModuleForm{
     ...
     */
    get entityClass(){
        return CookieAuthorizationModule;
    }
    ...
    renderAuxiliarySettings(){    
        return [
            <Label>{t("Cookie lifetime")}</Label>,
            <PositiveDurationInput
                {...this.getFieldProps('cookie_lifetime')}
                tooltip={t("After this certain period of time Cookie will be treated as expired and the user "+
                    "have to login again")}
            />
        ];
    }
