...
/** 
 * 	Allows the user to enter new pinwheel coordinates
 * 
 ...
 * 									will be provided.
 */
export default class PinwheelForm extends UpdateForm{


	/**
	 * 	Returns class of the changing entity
	 */
	get entityClass(){
		return Pinwheel;
	}
	...
	async fetchObject(lookup){
		return await Pinwheel.get({parent: window.application.functionalMap, id: lookup});
	}
	...
	render(){
		return (
			<DialogBox
				{...this.getDialogProps()}
				title={t("Pinwheel center properties")}
		...
		return (
			<div>
				<div className={style.data_section}>
					<Label>X = </Label>
					<TextInput {...this.getFieldProps('x')}/>
					<Label>Y = </Label>
					<TextInput {...this.getFieldProps('y')}/>
				</div>
				<div className={style.controls_section}>
					<PrimaryButton {...this.getSubmitProps()}>OK</PrimaryButton>
					<PrimaryButton
						type="cancel"
