...
/** The class represents module settings for the standard authorization module
 * 
 ...
 * 									will be provided.
 */
export default class StandardAuthorizationModuleForm extends ModuleForm{
     ...
     */
    get entityClass(){
        return StandardAuthorizationModule;
    }

    /** List of all entity fields that is allowed to modify using this form
     */
    get fieldList(){
        return ["max_failed_authorization_number"];
    }
    ...
    renderAuxiliarySettings(){
        return [
            <Label>{t("Maximum number of failed authorizations")}</Label>,
            <TextInput
                {...this.getFieldProps("max_failed_authorization_number")}
                tooltip={t("when the user will not be successfully authorized for more than this amount of time, throttling will be applied")}
                cssSuffix={style.max_failed_authorization_number_box}
            />
        ];
    }
