...
class TestUserProvider(APITestCase):
	...
	def test_create_user(self, input_data):
		"""
		Tests the user creating given that dealing with POSIX users is ON
		"""
	...
	def test_password_and_locked(self):
		...
		password_result = self.client.post(self.PASSWORD_RESET_PATH % user_id,
			**self.auth_headers)
	...
	def _assert_gecos_information(self, input_data, output_data, posix_user):
		self.assertEquals(output_data['login'], input_data['login'],
			"Logins are not the same")
		if posix_user.name != '':
			self.assertEquals(posix_user.name, input_data['name'],
				"User names are not the same")
		if posix_user.surname != '':
			self.assertEquals(posix_user.surname, input_data['surname'],
				"User surnames are not the same")
		if 'phone' in input_data and posix_user.phone != '':
			self.assertEquals(posix_user.phone, input_data['phone'],
				"User phones are not the same")

	def _delete_user(self, user_id):
		final_result = self.client.delete(self.USER_DETAIL_PATH % user_id,
			**self.auth_headers)
	...
	def _create_posix_user(cls, user_login):
