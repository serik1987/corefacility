...
class TestProfile(BaseTestClass):
    ...
    def test_profile_retrieve(self, token_id, login, name, surname, expected_response):
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            self.assertIn("is_locked", response.data, "The is_locked field must be hidden")
            self.assertIn("is_superuser", response.data, "The is_superuser field must be hidden")
            self.assertIn("is_support", response.data, "The is_support field must be hidden")
