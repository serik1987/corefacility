...
class TokenProvider(ModelProvider):
    """
    The base class for all internally generated tokens (i.e., authentication tokens, cookies etc.)
    """
    ...
    def wrap_entity(self, external_object):
        ...
        """
        token = super().wrap_entity(external_object)
        token._user = self._user_provider.wrap_entity(external_object.user)
        return token
