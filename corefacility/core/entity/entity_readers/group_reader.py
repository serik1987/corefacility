...
class GroupReader(RawSqlQueryReader):
    ...
    def apply_name_filter(self, search_string):
        ...
        :return: nothing
        """
        qf = SearchQueryFilter("core_group.name", search_string, must_start=True)
    ...
    def apply_user_filter(self, user):
        ...
        :return: nothing
        """
        user_source = "core_groupuser"
        user_condition = "ON (core_groupuser.group_id=core_group.id)"
        user_filter = StringQueryFilter("core_groupuser.user_id=%s", user.id)
